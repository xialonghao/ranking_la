...
//
    ...
    public function consume_money(){
        $token = $this->request->request('token');
        $user = \app\common\library\Token::get($token);
        $consumptiion_money = $this->Consumption->where('user_id="'.$user['user_id'].'"')->select();
        $consumemoney = 0;
    ...
    public function reach_graph(){
        ...
        for($i=1;$i<=substr($endDate,8);$i++){
          ...
          $num=count($wher_count[$i]);
            $wher_count[$i]['biao']=  $num/$key_sumcount;
        }
